...
class CausalSelfAttention(tf.keras.layers.Layer):
    def __init__(self, config):
        ...
        self.c_proj = tf.keras.layers.Dense(config.n_embd, config.n_embd)
        #Dropuout Layers
...
class MLP(tf.keras.layers.Layer):
    def __init__(self, config):
        super().__init__()
        self.c_fc = tf.keras.layers.Dense(config.n_embd, 4 * config.n_embd)
        self.c_proj = tf.keras.layers.Dense(4 * config.n_embd, config.n_embd)
...
#         self.h = [Block(Block) for _ in range(config.n_layer)]
...
class GPT(tf.keras.Model):
    ...
    def __init__(self, config):
        ...
        self.lm_head = tf.keras.layers.Dense(config.n_embd, config.vocab_size, bias=False)
    ...
    def configure_optimizers(self, train_config):
        ...
        optimizer = tf.keras.optimizers.experimental.AdamW(
            lr=train_config.learning_rate, weight_decay=train_config.weight_decay, beta_1=train_config.beta_1, beta_2=train_config.beta_2
        )
